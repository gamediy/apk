...
class Data {
  ...
  Data({required this.code, required this.msg,  required this.data});
...
class DioUtil {
  ...
  Future<void> _onRequest(RequestOptions options, RequestInterceptorHandler handler) async {
    ...
    if (options.path.contains("/user/")) {
      options.baseUrl = "http://127.0.0.1:4101/api";
    }
    ...
    var token=prefs.getString(Utils.TokenKey);
if(token!=null&&token!=""){
  ...
  Future<Data> request<T>(
    ...
    try {
      ...
      Utils.Toast("Error", e.message??"",false,SnackPosition.TOP);
      return e.response?.data;
    }
  }
